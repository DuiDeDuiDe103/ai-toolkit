...
| large_video | 大视频文件 | 🟢 可删 | 确认不再需要后可删 |
| large_game | 游戏数据 | 🟡 需确认 | 删除后游戏无法运行 |
| large_vm | 虚拟机文件 | 🟡 需确认 | 删除后虚拟机无法使用 |
| large_model | 模型文件 | 🟡 需确认 | 确认不再需要后可删 |
| large_image | 镜像文件 | 🟡 需确认 | 确认不再需要后可删 |
| large_other | 其他大文件 | 🟡 需确认 | 需用户判断 |
